...
namespace Oragon.Spring.Context.Support
{
    ...
    public class CodeConfigApplicationContextTests
    {
        ...
        {
            context.ScanWithAssemblyFilter(a => a.GetName().Name.StartsWith("Oragon.Spring.Core."));
            context.Refresh();
        ...
        public void Can_Filter_For_Assembly_Containing_Specific_Type_But_Having_NO_Definitions()
        {
            ...
            context.ScanWithAssemblyFilter(assy => assy.GetTypes().Any(type => type.FullName.Contains(typeof(Oragon.Spring.Core.IOrdered).Name)));
           context.Refresh();
