...
namespace Oragon.Spring.Aspects.Cache
{
    ...
    public abstract class BaseCacheAdvice : IApplicationContextAware
    {
        ...
        /// If the cache instance registered does not implement the <see cref="Oragon.Spring.Caching.ICache"/> interface.
        /// </exception>
        public ICache GetCache(string name)
        {
            ...
            if (cache == null)
            {
                throw new ArgumentException(String.Format(
                    "Cache with the specified name [{0}] does not implement the 'Oragon.Spring.Caching.ICache' interface.", name));
            }
            return cache;
        }
