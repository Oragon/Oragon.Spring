...
{
	/// <summary> 
	/// An unbounded priority <see cref="Oragon.Spring.Collections.IQueue"/> based on a priority
	...
	/// Multiple threads should not access a <see cref="Oragon.Spring.Collections.PriorityQueue"/>
	...
	public class PriorityQueue : AbstractQueue, ISerializable
	{
		...
		/// <summary>
		/// Creates a <see cref="Oragon.Spring.Collections.PriorityQueue"/> with the default initial capacity
		...
		/// <summary> 
		/// Creates a <see cref="Oragon.Spring.Collections.PriorityQueue"/> with the specified initial capacity
		...
		/// <summary> 
		/// Creates a <see cref="Oragon.Spring.Collections.PriorityQueue"/> containing the elements in the
		...
		/// instance of a <see cref="Oragon.Spring.Collections.PriorityQueue"/>, the priority queue will be sorted
		...
		/// knowledge that it is sorted or is another <see cref="Oragon.Spring.Collections.PriorityQueue"/>, in which
		...
		public virtual void GetObjectData(SerializationInfo serializationInfo, StreamingContext context)
		{
			...
			serializationInfo.AddValue("Oragon.Spring.Collections.PriorityQueuedata1", _queue.Length);

			// Write out all elements in the proper order.
			for (int i = 1; i <= _priorityQueueSize; i++)
			{
				serializationInfo.AddValue("Oragon.Spring.Collections.PriorityQueueData" + i, _queue[i]);
			}
		}

		/// <summary> 
		/// Reconstitute the <see cref="Oragon.Spring.Collections.PriorityQueue"/> instance from a stream (that is,
		/// deserialize it).
		/// </summary>
		/// <param name="serializationInfo">the stream</param>
		/// <param name="context">the context</param>
		protected PriorityQueue(SerializationInfo serializationInfo, StreamingContext context)
		{
			...
			int arrayLength = serializationInfo.GetInt32("Oragon.Spring.Collections.PriorityQueuedata1");
			_queue = new Object[arrayLength];

			for (int i = 1; i <= _priorityQueueSize; i++)
			{
				_queue[i] = serializationInfo.GetValue("Oragon.Spring.Collections.PriorityQueueData" + i, typeof (Object));
			}
		}
		#endregion
